...
## Document
Please refer to the [document](https://help.aliyun.com/document_detail/96909.html) if any issue. 
...
- Kubernetes 1.10 or newer cluster with RBAC (Role-Based Access Control) enabled is required
- Helm 2.10.0 or newer or alternately the ability to modify RBAC rules is also required
...
| `tracing-on-sls.enabled` | Specifies whether Tracing(jaeger on AliCloud Log Service) addon should be installed | true/false | `false` |
| `tracing-on-sls.storage.type` | Specifies the storage type for Tracing(jaeger on AliCloud Log Service) addon | aliyun-log | `aliyun-log` |
| `tracing-on-sls.storage.aliyun_sls.project` | Specifies the name of Log Service project which is used for Tracing(jaeger on AliCloud Log Service) addon if tracing-on-sls.enabled=true | project name |  |
| `tracing-on-sls.storage.aliyun_sls.logstore` | Specifies the name of Log Service logstore which is used for Tracing(jaeger on AliCloud Log Service) addon if tracing-on-sls.enabled=true| logstore name |  |
| `tracing-on-sls.storage.aliyun_sls.endpoint` | Specifies the name of Log Service endpoint which is used for Tracing(jaeger on AliCloud Log Service) addon if tracing-on-sls.enabled=true| endpoint  |  |
| `tracing-on-sls.storage.aliyun_sls.accesskey.id` | Specifies the accesskeyId to access your Log Service project which is used for Tracing(jaeger on AliCloud Log Service) addon if tracing-on-sls.enabled=true| accesskeyId |  |
| `tracing-on-sls.storage.aliyun_sls.accesskey.secret` | Specifies the accesskeySecret to access your Log Service project which is used for Tracing(jaeger on AliCloud Log Service) addon if tracing-on-sls.enabled=true| accesskeySecret |  |
